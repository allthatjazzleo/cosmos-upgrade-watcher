...
async fn main() {
    ...
    println!("Watching the following chains: {:?}", config.chain.watch_list);
    ...
    tokio::spawn(async move {
        ...
        loop {
            interval.tick().await;
            match fetch_and_update_metrics(&client_clone, &endpoint_clone, &watch_list_clone, &gauge_clone, &upgrades_clone).await {
    ...
    warp::serve(metrics_route)
        .run((config.prometheus.host.parse::<IpAddr>().unwrap(), config.prometheus.port))
        .await;
}
...
async fn fetch_and_update_metrics(
    ...
    let response = client.get(endpoint).send().await?.json::<Vec<ChainUpgrade>>().await?;
    let now = Utc::now();
